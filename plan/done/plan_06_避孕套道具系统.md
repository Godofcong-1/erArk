...
3. **处理函数**：`Script/System/Instruct_System/handle_instruct.py` 中加 `@add_instruct` 函数（8 个 handler 共用一个公共流程函数 `handle_condom_instruct_common`）：
   - **实行值判定**：先调 `instuct_judege.calculation_instuct_judege(0, 目标id, _("严重骚扰"))` 进行"严重骚扰"难度的实行值判定。判定必须在**打开选择面板之前**——面板确认时会直接结算数据，判定失败不能给出选择机会。返回 -1 直接中止；返回 0（判定失败）则结算为严重骚扰失败行为 `constant.Behavior.HIGH_OBSCENITY_ANUS` 后中止（与 `chara_handle_instruct_common_settle` 内置 judge 失败时的替换行为一致）
