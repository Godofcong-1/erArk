...
**2026-09-08 第一轮（与方案 §9.1 成对）：事件点名、互动对象的性格结算与死配置清理**

一、二期盘完之后回头盘三期。本期系统已由 Plan 23 接管，故对着当前代码盘。
符号级机扫（字段读写两向、函数调用点、模块级常量、配置号对注册表）**全部通过**，
缺口全在内容层——这轮新加的两个方向才扫得出来。

#### 实际改动

| 文件 | 类型 | 实际改动 |
| --- | --- | --- |
| `data/official_event/{婴儿,幼女,萝莉,通用,期末}.csv` | 改 | 266 条正文全部加上点名（269 处 `{Name}` + 38 处 `{TargetName}`）；16 条兄弟姐妹事件的 27 个选项追加 `CVE_A2_Growth` 与「，对方倾向同向」的后果提示 |
| `Script/System/Official_Event_System/official_event_panel.py` | 改 | 选项全不可选而跳过时改为 `record_event_done(uid, chara, 0)`，不再只 `continue` |
| `Script/System/Official_Event_System/official_event_handle.py` | 改 | `get_default_department_pick_list` 跳过 `subject == SUBJECT_CHARACTER` 的事件；`handle_ri_effect` 的两条报错改为点名目标数值 |
| `Script/System/Official_Event_System/ri_value.py` | 改 | 只读数值的两处守卫由静默返回改为打出被拦下的数值名 |
| `Script/System/Education_System/growth_event_handle.py` | 改 | `STAGE_ANY` 改为取 `official_event_handle.SUB_KEY_ANY`，两个同义常量合一 |
| `update.log` | 改 | v0.67 段追加 调整 2 条、修正 1 条 |
| `plan_22_生长养成系统_三期_方案.md` | 改 | §3.7.1 编号表补全 7 个编号并加「加入时间」列；§4.1 的 `once` 说明改为现状；新增 §9.1 |

#### 与方案的偏离（本期首轮，编号从 12 起接 §6.1 的 1~11）
...
14. **给互动对象写养成数值有一条红线：只能写给兄弟姐妹，不能写给同学。**
    `growth_handle.get_child_growth` 是**惰性创建**的，而「萝莉化」世界设定会给一大批
...
17. **`SUB_KEY_ANY` 与 `STAGE_ANY` 是同一个 0 的两个名字。** 通用层定义、
    教育层取域内别名（`STAGE_ANY = official_event_handle.SUB_KEY_ANY`），
    只留一个真相来源。这比删掉通用层那个更好——它是给日后别的部门用的。
...
`test_phase3_fix.py` **44 条断言全部通过**；事件表 token 校验（266 条 × 5 个前提列 + 4 个结算列
对三套注册表）仍为**零缺口**；连同一期的 `test_semester`（120 条）与二期的 `test_phase2_fix`（65 条），
共 **229 条全绿**。

断言的重点：

- **点名的三个维度**：266 条全部含占位符、`{Name}` 与 `{TargetName}` 能解析成正确的两个人、
  转换后交互对象已还原、无互动对象时落到博士。
- **「{Name}们」没有出现**——这是机械替换最可能留下的伤，专门断言了 0 处。
- **互动对象性格结算只落在兄弟姐妹事件上**：27 处全部所属事件的前提含 `sibling`，
  同学事件仍为 0 处（防的是给成年干员惰性创建养成数据）。
- **CSV 结构未被改坏**：列数一致、CRLF、无 BOM、无引号——正文是按整段原文替换的，
  一旦替换串不唯一就会写坏别的行，所以每次替换都断言了命中数为 1。

#### 尚未覆盖的验证

- 游戏内实际弹一条互动事件，看点名后的正文读起来是否自然（尤其是被重写的那 38 条）。
- 兄弟姐妹事件的 `CVE_A2_Growth` 对**成年结算**的实际影响未观察——
  它会让被动卷入事件的那个孩子也偏向某种性格，养到成年才看得出累积效果。
- `subject` 与 `SUBJECT_CHARACTER` 那条防御要等**第一条 `subject=0` 的部门事务事件**
  出现才真正生效，现在只验了它不误伤既有的 266 条。
- PO 词条仍需在有 `xgettext` / `polib` 的环境里补跑（本轮改了 266 条正文，词条变动很大）。
