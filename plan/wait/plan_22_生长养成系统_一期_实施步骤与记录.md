...
**2026-09-09 第八轮：修正教育区改建遗留的死场景位置（测试中发现）**

用户实机测试时崩在 `KeyError: '教\教室'`。那是**教育区改建之前**的单间教室，
一期第一步（`8f1071f4a`）把它拆成了 6 间理论教室、3 间实践教室与 1 个大礼堂，
场景早已不存在，而存档里还有角色站在里面。

#### 实际改动

| 文件 | 类型 | 实际改动 |
| --- | --- | --- |
| `Script/Core/save_handle.py` | 改 | `update_map` 末尾新增一段**无条件**执行的位置修复：把 position 指向已不存在场景的角色挪到所属区块的入口，并同步两边的 `character_list`；顶层 import 补 `map_handle` |
| `update.log` | 改 | v0.67 段追加 修正 1 条 |
| `plan_22_生长养成系统_一期_实施步骤与记录.md` | 改 | 新增本节 |

#### 实施中发现的偏离（编号接 62 往下）

63. **`update_map` 会删掉不存在的场景，却从不管站在里面的人。**
...
64. **修复必须无条件执行，不能放在 `change_map_flag` 里。**
    ...
    但各有 10 个角色的 position 还指着它；其余 13 个存档则相反，
    死场景键还在（所以暂时不崩），一旦哪次地图再变动就会立刻变成同样的坏账。

65. **回落目标选「同一区块的入口」而不是地图原点。**
    每个区块都有一个 `"0"` 号入口场景（`教\0` 是教育区入口），人还在原来的区块里，
    比一律扔回罗德岛大门更合理。整个区块都没了才回落到 `0\0`。

66. **范围是实测出来的，不是猜的。** 存档里像场景路径的字段不止 `position` 一个
    （`dormitory`、`permanent_dormitory`、`behavior.move_target`、
    `work.dormitory_admin_target_room`、`pl_ability.air_hypnosis_position`）。
    逐个扫过 15 个存档，**只有 `position` 中招**，所以本轮只修它——
    其余字段真出问题时，扫描脚本还在，照着再跑一遍即可。

#### 单元测试结果

新增 `test_dead_scene.py` **13 条全绿**，全部拿**真实存档 99** 跑（只读文件、不写盘）：
迁移前确有 10 人站在 `教\教室`、迁移后一个不剩、10 人都在 `教\0` 且进了它的角色名单、
没有人被漏在别的场景名单里、**崩溃点 `get_chara_now_scene_all_chara_id_list` 对这 10 人全部可用**、
再跑一次不再改动任何人（幂等）。
