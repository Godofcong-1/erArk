...
def update_tem_character(loaded_dict):
    ...
    for key, value in loaded_dict["character_data"].items():
        # 删除多余的夜半
        if value.adv == 473 and value.name == "夜半":
            delete_list.append(key)
        # 删除多余的蕾缪安
        if value.adv == 353 and value.name == "蕾缪安":
            delete_list.append(key)
    # 删除多余的角色
    for key in delete_list:
        # 角色数据
        value = loaded_dict["character_data"][key]
        # 找到角色的所处位置，从该位置的数据中删除该角色
        now_position = value.position
        now_scene_str = map_handle.get_map_system_path_str_for_list(now_position)
        now_scene_data = loaded_dict["scene_data"][now_scene_str]
        if key in now_scene_data.character_list:
            loaded_dict["scene_data"][now_scene_str].character_list.remove(key)
        # 如果该角色已经获得，则从npc_id_got中删除
        if key in loaded_dict["npc_id_got"]:
            loaded_dict["npc_id_got"].remove(key)
        # 获取角色预设，且该预设重复，则删除多余的预设
        npc_tem_cid = key - 1
        tem_npc_data_before = loaded_dict["npc_tem_data"][npc_tem_cid - 1]
        tem_npc_data_now = loaded_dict["npc_tem_data"][npc_tem_cid]
        tem_npc_data_next = loaded_dict["npc_tem_data"][npc_tem_cid + 1]
        # 如果角色预设重复，或者角色预设与前后角色相同，则删除该预设
        if tem_npc_data_now.Name == value.name or (tem_npc_data_now.Name == tem_npc_data_next.Name or tem_npc_data_now.Name == tem_npc_data_before.Name):
            loaded_dict["npc_tem_data"].pop(npc_tem_cid)
            update_count += 1
        # 删除角色数据
        del loaded_dict["character_data"][key]
        # 将之后的所有角色的cid减1，key也要减1，并考虑到遍历中删除元素会造成长度变化的问题
        for other_key in list(loaded_dict["character_data"].keys()):
            if other_key > key:
                loaded_dict["character_data"][other_key - 1] = loaded_dict["character_data"].pop(other_key)
                loaded_dict["character_data"][other_key - 1].cid -= 1
    ...
    for key, value in loaded_dict["character_data"].items():
        ...
        while 1:
            ...
            if tem_cid >= len(loaded_dict["npc_tem_data"]):
                ...
                i = 0
                print(f"debug 超出预设数量，新增了空白预设，序号为{value.cid - 1}")
                break
            tem_character = loaded_dict["npc_tem_data"][tem_cid]
            # 循环的终止条件为找到干员编号相同的角色
            if value.adv == tem_character.AdvNpc:
                print(f"debug 找到匹配的角色，序号为{tem_cid}")
                break
            i += 1
            print(f"debug i = {i}, adv = {value.adv}, tem_cid = {tem_cid}, tem_character.AdvNpc = {tem_character.AdvNpc}, value.name = {value.name}")
        # 如果i不为0，说明序号不一致，需要修正
        if i != 0:
