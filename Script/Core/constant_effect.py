...
class BehaviorEffect:
    ...
    """ 用事件文本代替指令文本，限指令后置 """
    SEX_CLASS_MODE_ON = 10014
    """ 系统量_基础 开启性技实操课（课堂H）模式（Plan 22 四期） """
    SEX_CLASS_MODE_OFF = 10015
    """ 系统量_基础 关闭性技实操课（课堂H）模式（Plan 22 四期） """
    ...
    """ 特殊flag_基础 自身宿舍管理员阶段设为0（到岗整理） """
    DORMITORY_ADMIN_PHASE_TO_1 = 374
    """ 特殊flag_基础 自身宿舍管理员阶段设为1（移动处理） """
    ...
    """ 指令_专用结算 （教学用）按课表查当节所授科目（查不到回落学识），自己增加该科目的习得和经验；当前场景里在听课的学生按师生等级差增加该科目的习得和经验（同一节只结算一次），NPC 教师只发给自己的学生（本节课表指向这间教室、听课节次与自己开讲的节次相同，Plan 32），如果玩家是老师则再加好感和信赖 """
    ...
    ASK_COPY_KEY = 543
    """ 指令_专用结算 （要求复制钥匙用）获取交互对象的所在楼层的钥匙 """
    MAKE_FOOD_ADD_ADJUST = 544
    """ 指令_专用结算 （制作食物用）根据当前行为中的菜谱难度等级，动态获得料理经验与习得 """
    TASTE_WINE_ADD_ADJUST = 545
    """ 指令_专用结算 （品酒用）根据酒量随机选一个酒，并结算部分饮用效果 """
    TAKE_CHARA_EGGS_SETTLE = 546
    """ 指令_专用结算 （拿走产下的卵用）拿走交互对象的全部未鉴定卵，卵数据留原处标记被拿走并写入玩家临时持有索引 """
    IDENTIFY_HELD_EGGS_SETTLE = 547
    """ 指令_专用结算 （鉴定持有的卵用）鉴定玩家临时持有的全部卵并经索引回写原角色卵数据，无精删卵、受精进入孵化 """
    SELF_STUDY_ADD_ADJUST = 548
    """ 指令_专用结算 （自习用）本节无教师时的降级行为，按自习基础值增加习得与所选科目的经验，不受师生等级差影响 """
    SKIP_CLASS_ADD_ADJUST = 549
    """ 指令_专用结算 （翘课用）置翘课flag，小幅回复心情，本节不获得任何学习收益 """
    CHECK_REPORT_CARD_ADD_ADJUST = 550
    """ 指令_专用结算 （检查成绩单用）有未查看的新成绩单时输出那一份并清除成绩单flag，否则输出该女儿本学期截至目前的各科成长与出勤；按成绩档位加好感与亲密 """
    ...
    """ 指令_专用结算 （听课用）学生坐下听课时，本节教师判能到岗即从学生这一侧结算本节课的收益与出勤；同一节只结算一次（Plan 22 第五轮加，Plan 31 放宽） """
...
class SecondEffect:
    ...
    """ 属性_经验 自己增加1H经验 """
    LAY_SOFT_EGG = 621
    """ 生育 无壳卵生体外排卵：生成一枚体外无壳卵并转移体内精液 """

    SHOW_OFF_STUDY = 622
    """ （炫耀用）女儿向博士炫耀最近升级的能力，加好感与亲密，并清空待炫耀记录 """
    CAUGHT_SKIP_CLASS = 623
    """ （翘课被抓用）翘课中的孩子与玩家同场景，清翘课flag并加抑郁与恐怖 """
