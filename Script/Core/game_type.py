...
class DIRTY:
    ...
    def __init__(self):
        ...
        """
        # 注意：大浴场衣柜与宿舍衣柜共用这一份污浊数据，
        # 依赖“同一角色同一时刻只会有一个衣柜里存着衣服”这条不变量。
...
class PREGNANCY:
    ...
    def __init__(self):
        ...
        self.growth_acceleration_days: float = 0.0
        """ 成长加速药对本角色（作为孩子）累计的额外成长天数：float累计（每次为当前阶段剩余天数的30%）、参与天数换算时向下取整；
            参与婴儿→幼女(90)/幼女→萝莉(270)/萝莉→少女(450)的天数换算与面板预计日期；
            由 pregnancy_handle.get_child_grow_day 统一读取；出生时为0，跨阶段不清零（成长为少女后不再消费） """
        self.fetus_count: int = 0
...
class CHILD_GROWTH:
    ...
    主要挂在孩子身上，成年后保留供查阅。出生时不一定创建：母亲胎教值大于 0 时由胎教折算（baby_growth_handle.settle_prenatal_to_child）创建，
       否则等照料、选课、事件结算等第一次写入时才惰性创建。
    普通干员也可以选课学技能（已确认口径 24），此时同样会被惰性创建出本结构体：学生岗的出勤、缺课、翘课、实操课次数与翘课 flag 都记在这里，
       性格倾向、照料、胎教、成绩单等其余养成字段保持初值。
    ...
    def __init__(self):
        ...
        """ 上一次已结算过课堂收益与出勤的 [日期序数int, 节次int]，同一节课只结算一次（Plan 22 第五轮）。
            学生坐下听课时自己这一侧的结算（557，本节教师判能到岗即结算，Plan 31）与教师开讲时的广播（512）都会走到，靠它去重；
            实操课记出勤时也写它（Plan 32），同一节不再另记缺课 """
...
class ACTION_INFO:
    ...
    def __init__(self):
        ...
        """ 角色睡觉中被吵醒状态的结束时间，在此之前角色不会产生睡觉需求 """
