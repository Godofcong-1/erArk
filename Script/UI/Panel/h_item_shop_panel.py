...
class HItemShopPanel:
    ...
    def draw(self):
        ...
        while 1:
            ...
            # 绘制粉色凭证
            money_text = f"当前持有粉色凭证：{str(cache.base_resouce.materials_resouce[5])}"
...
class BuyItemByItemNameDraw:
    ...
    def buy_item(self):
        ...
        line.draw()
        item_config = game_config.config_item[self.text]
        if cache.base_resouce.materials_resouce[5] >= item_config.price:
            self.character_data.item[self.text] += 1
            cache.base_resouce.materials_resouce[5] -= item_config.price
