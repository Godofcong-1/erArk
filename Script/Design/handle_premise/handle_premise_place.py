...
@add_premise(constant_promise.Premise.IN_PRACTICE_ROOM)
def handle_in_practice_room(character_id: int) -> int:
    """
    校验角色是否在实践教室
    Keyword arguments:
    character_id -- 角色id
    ...
    now_scene_str = map_handle.get_map_system_path_str_for_list(now_position)
    now_scene_data = cache.scene_data[now_scene_str]
    if "Practice_Room" in now_scene_data.scene_tag:
        return 1
    return 0


@add_premise(constant_promise.Premise.NOT_IN_PRACTICE_ROOM)
def handle_not_in_practice_room(character_id: int) -> int:
    """
    校验角色是否不在实践教室
    Keyword arguments:
    character_id -- 角色id
    ...
    int -- 权重
    """
    if handle_in_practice_room(character_id):
        return 0
    return 1


@add_premise(constant_promise.Premise.IN_AUDITORIUM)
def handle_in_auditorium(character_id: int) -> int:
    """
    校验角色是否在大礼堂
    Keyword arguments:
    character_id -- 角色id
    ...
    now_scene_str = map_handle.get_map_system_path_str_for_list(now_position)
    now_scene_data = cache.scene_data[now_scene_str]
    if "Auditorium" in now_scene_data.scene_tag:
        return 1
    return 0


@add_premise(constant_promise.Premise.NOT_IN_AUDITORIUM)
def handle_not_in_auditorium(character_id: int) -> int:
    """
    校验角色是否不在大礼堂
    Keyword arguments:
    character_id -- 角色id
