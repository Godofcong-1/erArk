...
本文件里有一批常量是**从配置表现算**的（科目表、阶段名、性格倾向名），
...
""" 成长链的四个年龄素质id：101婴儿 / 102幼女 / 103萝莉 / 104少女。
    这四个id是成长链的**定义本身**（`pregnancy_handle` 按天数逐级换素质），配置里推不出来，
...
""" 四个阶段的显示名，从 Talent.csv 现取。
    不要再包一层 `_()`：`game_config` 载入时已对所有 name 列做过翻译，再包一次反而对不上词条 """
...
""" 教育区在 Facility_effect.csv 中的设施名（该表按名字索引）。
    必须过 `_()`：`config_facility_effect_data` 的键是 `game_config` 翻译过的 name，
...
""" 归在上面两个类型里、但**不是课程科目**的能力。
    90 隐蔽的 ability_type 也是5，可它是隐奸系统的熟练度，不是能开课教的东西。
...
""" 课型在格子里的单字缩写。
    只有个人式的三种课型需要它——班级式（理论/实践/公开）的格子里写的是教室名，
...
""" 体育课的四处地点（一期方案 §3.21）：地点名 → (场景标签, 行为en_name, 状态cid)。
    用固定表而不是查配置，是因为这四处各自对应不同的既有行为，没有一张现成的表能推出来。
    **键必须过 `_()`**：它要拿去和 `scene_data[].scene_name` 逐一比对（`schedule_handle` 里
       体育课要精确到具体那间房），而场景名在载入时过了翻译（`map_config.py:64`）。
       键同时也是存进个人课表的 target，与全局课表存教室名是同一套做法。
    场景数据实际走的是 pickle 缓存（`map_config.py:22`），只有冷构建那一次才过翻译，
       所以**场景名跟的是「缓存生成时的语言」而不是当前语言**。先用中文玩过再把 config.ini
       改成英文，缓存不会重建，全岛地名都还是中文——那是整个游戏的既有问题，不是本表独有。
    值里的场景标签与行为 en_name 都不翻译。
...
""" 教室名末尾中文数字的**数值**序。默认的 sorted() 按 Unicode 码位排，
    ...
    过 `_()` 是为了让译者能给出该语言下对应的一串序号字符（如 "1234567890"），
       教室名本身是翻译过的场景名 """
...
""" 实习课不开放的岗位：教师与学生是孩子自己在学校里的身份，作为「实习」语义重复（一期方案 §3.21）。
    从上面两个常量派生而不是再写一遍 151/152，改岗位id时只动一处 """
...
""" 娱乐配置「跟随母亲」的cid（Entertainment.csv）。它没有固定地点，执行走 class_ai 的见学分支；
    ...
    2026-09-09 由 176 改为 153（原编号排在大浴场段 17x）：旧存档里的旧编号由 save_handle 读档时换算 """
...
""" 幼女没排日程（时段为「自由选择」）时每日随机的默认池：每个时段在过家家 / 自由玩耍之间随机
    ...
    只管晚上等不在课表节次内的时间：白天没课的节次幼女默认见学（class_ai.judge_should_follow_mother），
       日程明确排了活动的时段则做那个活动 """
...
""" 日程时段的「自由选择娱乐活动」：槽位存 0，表示该时段不改写、保留当天的随机娱乐。
    它不是 Entertainment.csv 里的一行，只是 0 这个哨兵值的显示名；不满足活动条件的孩子也会退回到它 """
...
""" 预设模板各时段的娱乐**名字**（不是cid）。
    写名字而不是写cid，是因为 Entertainment.csv 的编号会随内容增删漂移，
...
""" 四对性格倾向 → (正数侧素质id, 负数侧素质id)：
    ...
    三期的养成事件是它的主要写入方，四对全为0时面板显示「尚未形成」 """
...
""" 四对性格倾向的显示名，从上面那张素质id表现取 Talent.csv 的名字。
    这样两张表**不可能对不上**——它们原先分居 growth_panel 与 growth_handle 两个文件，
...
REPORT_GRADE_NONE = -1
""" 档位读口的空值：这孩子还没有过成绩单。
    绝不能用0兜底——0是「优秀」，会让全岛没上过学的人都通过优秀档的口上前提 """
REPORT_GRADE_NAME = {
...
""" 评为优秀所需的本学期升级科目数下限。光靠不缺课评不上优秀，还得真学出东西来 """
GOOD_RATE = 70
...
""" 期末事件的**保留**子桶键（对应 data/official_event/期末.csv 的 sub_key 列）。
    ...
    绝不能用 0 或 101~104：get_candidate_event_list() 每天翻的正是
...
""" 新建/重命名模板时允许输入的最大长度。
    与 COLUMN_WIDTH_NAME 对齐：模板表的名字列宽是10显示列，
       输入更长的名字会把后面三个时段列整体挤右，一行对不齐 """
COLUMN_WIDTH_SLOT = 20
""" 模板表三个时段列各自的显示列宽。
    pad_display_width 不截断，列宽必须≥该列最长内容：
