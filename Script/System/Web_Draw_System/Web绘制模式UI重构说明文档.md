...
#### 2.2.6 交互类型面板（左侧）
- **位置**：主画面区的左侧边缘，垂直上对齐（2026-03-17更新，原为垂直居中）
...
- **布局实现**：
  ...
  - 整组大类卡片从容器顶部开始排列（2026-03-17更新，原为垂直居中）：
    - 固定顶部偏移：`TOP_OFFSET = 0.5rem`
    - 每个大类卡片位置：`top = TOP_OFFSET + index × 5.3125rem`（5.3125rem = 卡片高度 4.375rem + 间距 0.9375rem）
  - 小类列表位置在对应大类卡片下方，同样使用绝对定位
...
- `.new-ui-main-scene` 设置了 `min-height: 32rem` 以确保交互按钮能够完整显示（7个卡片×70px + 6个间距×15px ≈ 580px，2026-03-17降低）
...
游戏事件系统支持分支选项，在Web模式下需要特殊处理以避免阻塞。

#### 6.4.1 事件选项概述
- **触发场景**：当事件包含子事件选项时，需要玩家选择后继续
- **事件类型**：
  - **普通子事件**：事件前提中包含 `option_son`
  - **多层嵌套事件**：事件前提中包含 `CVP_A1_Son` 格式
  - **角色DIY事件**：特定角色的自定义事件分支
...
- **组成部分**：
  - 上方：事件描述文本（如果有）
  - 下方：可选的事件选项列表
- **样式**：半透明深色背景 + 圆角边框 + 选项高亮效果

#### 6.4.3 数据流程
1. **事件文本阶段**：
   - `DrawEventTextPanel` 判断事件类型
   - 非子事件文本保存到 `cache.pending_event_text` 供选项弹窗使用
   - 子事件文本直接作为对话文本输出
2. **选项显示阶段**：
   - `event_option_panel` 收集可用子事件
   - 调用 `_handle_web_event_options()` 发送选项到前端
   - 事件文本和选项文本同时添加到 `web_instruct_texts` 缓存（用于文本回溯）
3. **等待选择阶段**：
   - 前端显示事件选项弹窗
   - 后端通过 `get_event_option_response()` 轮询等待
4. **选项确认阶段**：
   - 前端发送 `submit_event_option` 事件
   - 后端设置 `character_data.event.son_event_id`
   - 关闭弹窗，继续事件结算
5. **子事件输出阶段**：
   - 子事件文本作为对话文本输出
   - 添加到 `web_instruct_texts` 和对话框队列

#### 6.4.4 技术要点
- **避免阻塞**：Web模式下不使用 `WaitDraw`/`LineFeedWaitDraw` 的键盘等待
- **文本处理**：事件文本经过 `talk.code_text_to_draw_text()` 转换代码词语
- **对话输出**：子事件文本通过 `add_dialog_text()` 添加到对话框队列
- **文本回溯**：所有事件文本和选项添加到 `web_instruct_texts` 供回溯查看

#### 6.4.5 相关代码路径
- 事件选项面板：`Script/UI/Panel/event_option_panel.py`
- 事件文本绘制：`Script/UI/Panel/draw_event_text_panel.py`
- WebSocket事件处理：`Script/Core/web_server.py` 中的 `emit_event_options()`、`handle_submit_event_option()`
- 前端弹窗：`static/game.js` 中的 `showEventOptionsModal()`
- 样式定义：`static/css/style.css` 中的 `.event-options-modal`

### 6.5 结算选项弹窗系统（2026-03-20新增）

在结算过程中（如射精判断），需要显示选项按钮供玩家选择。由于结算遮罩的 z-index 为 10000，普通按钮会被遮挡无法点击。因此需要专门的结算选项弹窗来处理这类交互。

#### 6.5.1 触发场景
- **结算模式判断**：当 `cache.web_text_recording_flag` 为 True 时（行为循环/结算中）
- **典型场景**：射精忍耐选择（"忍住"/"射出"按钮）、其他需要玩家在结算中做出选择的情况

#### 6.5.2 结算选项弹窗组成
- **外层遮罩**：半透明黑色背景，z-index: 15000（高于结算遮罩的10000）
- **弹窗内容区**：
  - 上方：描述文本（如"要射精了，是否要忍住射精，忍耐次数越多则射精量越多……"）
  - 中间：标题"请选择"
  - 下方：按钮列表
- **滚动机制**：当按钮数量超过4个时，按钮容器启用滚动条

#### 6.5.3 数据流程
1. **按钮检测阶段**：
   - `askfor_all()` 检测到 `cache.web_text_recording_flag` 为 True
   - 调用 `_handle_settlement_buttons()` 处理结算按钮
2. **描述文本保存**（2026-03-21更新）：
   - 由于 `update_game_state()` 会清空 `web_other_texts`，需要在调用前保存
   - 在 `askfor_all()` 中先复制 `web_other_texts` 到 `saved_other_texts`
   - 然后将 `saved_other_texts` 传递给 `_handle_settlement_buttons()`
3. **按钮提取阶段**：
   - 从 `cache.current_draw_elements` 提取 `type == "button"` 的元素
   - **描述文本获取**：从传入的 `saved_other_texts` 中获取最后一段有效文本作为描述
   - 也可通过 `cache.web_settlement_description` 预设描述文本（优先级更高）
4. **发送到前端**：
   - 调用 `emit_settlement_buttons(buttons, description_text)` 发送按钮数据
   - 数据格式：`{visible: true, buttons: [{text, return_text, tooltip}], description_text}`
5. **等待选择**：
   - 后端通过 `get_settlement_button_response()` 轮询等待
   - 前端显示结算选项弹窗
6. **选择确认**：
   - 用户点击按钮，前端发送 `settlement_button_selected` 事件
   - 后端接收 `return_text`，执行关联命令
   - 调用 `emit_settlement_buttons(None)` 关闭弹窗
7. **状态清理**（2026-03-21新增）：
   - 清空 `cache.web_settlement_description`
   - 从 `cache.current_draw_elements` 中移除按钮元素，避免下次重复显示
   - 清空 `cache.web_other_texts`，避免描述文本残留

#### 6.5.4 z-index 层级关系
| 层级 | z-index | 元素 |
|------|---------|------|
| 最高层 | 15000 | 事件选项弹窗、结算选项弹窗 |
| 次高层 | 10000 | 结算遮罩 |
| 正常层 | 1-999 | 主界面元素 |

#### 6.5.5 相关代码路径
- 缓存字段：`Script/Core/game_type.py` 中的 `web_settlement_description`
- 按钮处理：`Script/Core/flow_handle_web.py` 中的 `_handle_settlement_buttons()`
- WebSocket函数：`Script/Core/web_server.py` 中的 `emit_settlement_buttons()`、`get_settlement_button_response()`
- 前端弹窗：`static/game.js` 中的 `showSettlementButtonsModal()`、`hideSettlementButtonsModal()`
- 样式定义：`static/css/style.css` 中的 `.settlement-buttons-modal`
