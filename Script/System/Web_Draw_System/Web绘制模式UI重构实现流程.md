...
- [x] 目录模式（2026-09-11 调整）：`body_analysis_ensemble.py <目录>` 只识别文件名以 `_全身` 结尾的全身立绘，跳过 `_半身`/`_头部` 图层与差分图，JSON 写到图片同目录的 `{文件名去掉_全身}_body.json`（与 character_renderer 按立绘查找关键点文件的规则一致）；新增 `-r` 递归子目录、`--all-images` 恢复不限后缀、`--dry-run` 只列清单；已用于 `image/立绘/女儿` 的 15 张种族女儿全身图
- [x] 读图去杂色（2026-09-11）：立绘 PNG 透明像素里多残留抠图杂色（抽样 68 张全身图中 58 张），原先 `IMREAD_COLOR` 丢掉透明通道会把杂色当背景；现由 `load_image_bgr` 按 alpha 铺黑底后再识别。45 张样图对比：黑底与旧读法的模型间一致性、置信度相当，关键点平均位移约 0.55% 图高，白底、灰底一致性更差；已有 JSON 均经人工修正，未重跑，新读法只用于之后新生成的 JSON
