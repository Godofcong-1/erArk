...
### 3.4 指令排序机制（2026-02-14更新）

Web模式下，指令列表的显示顺序按照 `Script/System/Instruct_System/Instruct.py` 中的定义顺序排序，而非 `@add_instruct` 装饰器的执行顺序。

#### 实现原理

```python
# 位于 Script/Design/web_interaction_manager.py
from Script.System.Instruct_System.Instruct import Instruct

# 生成排序映射（使用 __dict__ 保持定义顺序）
...
以下函数返回的指令列表均按此顺序排序：
- `get_instructs_by_minor_type()` - 获取指定小类型下的指令
- `get_instructs_by_body_part()` - 获取指定身体部位关联的指令
