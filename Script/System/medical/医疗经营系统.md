...
- **治病提示开关**：状态文字与“切换提示”按钮合并为居中行，玩家点击即可切换开关，并在同一模块下方常驻提示“医疗技能的能力等级越高，就可以获得更多的提示。”开启后依据玩家医疗技能等级（记为 `player_med_level`）逐级决定检查菜单的高亮范围，高亮表现为对应系统/部位/检查按钮切换为醒目的配色：
  - `>=1`：提示患病生理系统；
  - `>=2`：提示具体部位；
  - `>=3`：高亮轻症正确检查方法；
  - `>=5`：高亮中症正确检查方法；
...
- **操作按钮**：
  ...
  - “开药并治疗”：在确诊≥1条或检查次数用尽时启用。执行后发放诊疗收入，病人状态从 `in_treatment_player` 进入 `waiting_medication` 并触发常规用药流程（条件未满足时按钮不渲染）。
