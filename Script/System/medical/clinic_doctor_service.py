...
def dispatch_medical_doctors(
    ...
    *,
    ...
    参数:
        clinic_target (Optional[int]): 期望的门诊医生数量，缺省沿用历史目标。
        hospital_target (Optional[int]): 期望的住院医生数量。
        target_base (Optional[game_type.Rhodes_Island]): 目标基地对象。
        assign_work_func (Optional[Callable[[int, int], bool]]): 自定义岗位分配回调，允许直接调整角色工作。
        refresh_callbacks (Optional[Iterable[Callable[[], None]]]): 重分配后需要执行的刷新回调集合。
    返回:
        Dict[str, int]: 包含目标值、实际分配、缺口与岗位调整次数的统计信息。
    """
    # 若无法确定基地则返回缺省统计。
    rhodes_island = medical_core._get_rhodes_island(target_base)
    if rhodes_island is None:
    ...
    # 收集符合条件的干员，记录其当前工作与医疗能力等级，供排序使用。
    ...
    # 针对住院与门诊分别构建优先队列，优先保留已经在对应岗位上的医生。
