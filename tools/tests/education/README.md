...
| `test_class_ai.py` | 翘课概率表、教师可用性、缺课去重、体力闸与心情闸、必修实操课豁免、六种课型派发、见学母亲有效性与回落链、预到岗边界 |
| `test_growth.py` | 速度曲线、教育区加成、成长停滞减半、学生 / 教师 / 见学结算、成年结算、养成数值读写、两份候选名单 |
| `test_semester.py` | 学期时间、本学期出勤、四档判据、等级增量、学期切换幂等、成绩单历史上限与正文 |
| `test_schedule_template.py` | 预设模板、增删改、套用与覆盖、need 校验、课表优先、每日改写 |
...
| `test_panels.py` | 页签容器、全局课表（格子编辑、临时课编辑、必修名单）、个人课表（分页、选课、复制、清空）、养成总览（成绩单翻页、待处理）、日程模板、`Student_Tab_Bar`、Web 适配器冒烟 |
| `test_talk_data.py` | 教育相关口上的覆盖、行为与前提 token 逐条校验、正文卫生、`tools/official_event_check.py`、配置一致性、ArkEditor 表同步 |
| `test_save_compat.py` | `CHILD_GROWTH` 字段级回填与成绩单迁移、`save/` 下全部真实存档只读载入后的结构、学期结算幂等、四个面板可画 |
