...
```bash
.conda\python.exe tools/tests/education/run_all.py            # 全部 15 个文件、1126 条断言，约 1 分钟
...
| `test_semester.py` | 学期时间、本学期出勤、四档判据、等级增量、学期切换幂等、成绩单历史上限与正文；Plan 29：`settle_course_attend` 的守卫、去重与节次外不记，只排个人式课的孩子按档评（不再无课可评、缺一节按实际算），改岗萝莉照出成绩单并带不全的理由（写明改任的岗位 / 岗位为无、正文与学期切换冻结的快照都有、学生岗为空、旧快照无 `reason` 键） |
