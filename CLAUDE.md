...
- **`.claude/workflows/`** (not under `tools/`, listed here for discovery): saved Workflow scripts `system-review-find.js` / `system-review-implement.js`, driven by the skill `system-review-round` (`.github/skills/system-review-round/`, which also ships `dead_code_scan.py` and `repro_template.py`)
