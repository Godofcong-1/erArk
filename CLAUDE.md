...
- **`Script/System`** - Self-contained subsystems, each directory carrying its own design docs (`.md`): `Cooking_System`, `Dormitory_System`, `Education_System` (schooling & child-raising: timetables, classes, growth), `Field_Commission_System`, `First_Record_System` (sexual first-experience record system & panel), `Instruct_System` (instruction panel & categories), `Item_System`, `Medical_System`, `Official_Event_System` (official-duty events), `Pregnancy_System`, `Sex_System`, `Web_Draw_System` (web rendering)
...
- **`tools/map_aa_check.py`**: Pixel-level alignment checker for the AA maps in `data/map/*/Map`. Map layout is decided by the **font's real advance widths**, not by `wcwidth` — box-drawing characters and ASCII spaces belong to two incommensurable metric families that differ at some font sizes (including the size the current `config.ini` produces). Never validate an AA map by counting columns; run this instead. Supports `--tol N` and `--list-bad`, and exits non-zero on failure.
- **`tools/map_aa_build_training.py`**: Generator for `data/map/训练/Map`. That map pads with **U+2002 EN SPACE** (never ASCII space) so its walls stay aligned at every font size; U+2002 is visually identical to a normal space, so the file must be regenerated by this script rather than hand-edited.
- **`.claude/workflows/`** (not under `tools/`, listed here for discovery): saved Workflow scripts `system-review-find.js` / `system-review-implement.js`, driven by the skill `system-review-round` (`.github/skills/system-review-round/`, which also ships `dead_code_scan.py`, `repro_template.py` and `wf_watch.py` — the last reports whether a running Workflow's agents have finished, are stuck, or are waiting on a hung command)
...
- **[系统复查工作流](.github/prompts/数据处理工作流/系统复查工作流.md)** - Repeated full review of an implemented system: dossier, Workflow multi-agent find/verify and implement/audit, plan double-docs, wrap-up (skill `system-review-round`)
