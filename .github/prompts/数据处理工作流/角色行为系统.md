...
- **决策优先级**（`find_character_target`，各段只在前面都没派出行为时执行）:
  1. 高优先 type 0（跟随、继续移动、爆睡、起床等）
  2. 需求链 12 → 13 → 11，助理链 42 → 43 → 41
  3. 见学（幼女 / 萝莉跟随母亲，`class_ai.judge_follow_mother_state_machine`）
  4. 工作链：外层闸只判「有工作」，时间窗由各目标行自己的时间前提决定（Plan 24）
  5. 娱乐链 32 → 33 → 31（需在娱乐时间）
  6. 其余全部目标行的全表搜索
- **工作链的两种形态**（`WorkType.csv` 的 `auto_ai` 列）:
  - `auto_ai=1`：人在岗位地点（`place` 场景名或 `place_tag` 场景标签）→ 自动 AI 的「工作」数据，不在 → 检索 type 22；工作准备走自动 AI 的「移动」数据。自动 AI 路径自带 `work_time` 前提，非工作时间不会触发
  - `auto_ai=0`：先检索 type 22「进行工作」，再检索 type 21「工作准备」；每行自己带时间前提（`to_work_time` / `work_time`），只有体检链的 4 行不带
  - 教师（151）/ 学生（152）属于后者：课表行（`target.csv` 组 07 / 08）不看星期，周日排的课照常上；前提从 `class_ai.get_teacher_duty / get_course_stage` 取互斥状态，见 [生长养成系统](生长养成系统.md) §4

#### 3.3 前提系统 (`handle_premise/`)
- **作用**: 验证行为和事件的执行条件
- **主要函数**:
...
- **核心文件**:
  ...
  - `Script/Design/handle_premise/`: 前提系统
  - `Script/Design/event.py`: 事件系统

- **配置文件**:
  - `Script/Core/constant/Behavior.py`: 行为常量定义
  - `data/csv/Behavior_Data.csv`: 行为配置数据
  - `data/csv/Behavior_Effect.csv`: 行为效果配置

- **结算实现**:
  - `Script/Settle/default.py`: 默认结算实现
  - `Script/Settle/Second_effect.py`: 二段行为结算
