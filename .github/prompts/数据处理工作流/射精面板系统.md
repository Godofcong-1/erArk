...
- `Ejaculation_Panel.draw`：先查 `soft_egg_handle.get_soft_eggs_in_scene(玩家位置)`，当前地点有体外无壳卵时**无视"自动射精"设置强制进入 `draw_choose_part`**；交互对象为自己（自慰）时进入 `draw_choose_part(self_mode=True)`——不列身体/服装部位，只列卵按钮与 `[照常射出]`（走原 `shoot_here(6, 0)`）。
- `draw_choose_part(self_mode=False)`：服装按钮之后追加一行卵按钮 `[卵{编号}]{母亲}排出的卵（{X}ml）`（每 4 个换行）→ `shoot_soft_egg(egg_id)` → `ejaculation_flow(-1, 0, soft_egg_id=egg_id)`。
- `ejaculation_flow(..., soft_egg_id=-1)`：非避孕套且 `soft_egg_id>=0` 时把射精量全部加到该卵（`soft_egg_handle.add_semen_to_soft_egg`），文案"在{母亲}排出的卵上{射精文本}"；**不**更新部位污浊、精液流动、受精概率与 `shoot_position`/`shoot_position_body`（保持 -1，部位射精类前提天然为假）；避孕套分支照旧进套；成就照旧。
