...
- **收费系数**：玩家可设置 `medical_price_ratio`（默认 1.0）。影响病人刷新量、诊疗收入与药物结算收益。收费系数的调整采用"第二天生效"机制：玩家本次调整的系数会暂存为待调整值，当天的各项结算仍使用当前生效的收费系数，直到第二天结算时才会应用新系数。
...
- `cache.rhodes_island.medical_price_ratio`：当前生效的收费系数。
- `cache.rhodes_island.new_medical_price_ratio`：待调整的收费系数，非 0 时表示玩家已设置新值，将在第二天结算时生效。
...
3. 刷新数量计算：
  ...
  infection_ratio = cache.country.country_infection_patient_ratio.get(current_country_id, 1.0)
  refresh_count = floor(base_count * ratio_penalty * infection_ratio)
   ```
  - `infection_ratio` 由 `country_infection_patient_ratio` 提供，默认 1.0；用于体现不同国家的感染强度对病人数量的影响。
4. 病情与并发症生成：
...
- 输出流程：
  ...
  5. 检查是否存在待调整的收费系数（`new_medical_price_ratio`），若有则将其应用为当前收费系数并清空待调整值。
  6. 按更新后的收费系数刷新病人列表。
...
- **检查方法模块**：在尚未找全正确并发症且 `used_checks < player_talk_level * 2` 时绘制，其中 `player_talk_level` 代表博士当前的话术能力等级（Ability ID 40）。
  ...
  - 允许多选，最大可并行选择数量等于病人并发症数。总检查次数上限为 `max(1, player_talk_level * 2)`，即博士每级话术能力额外提供 2 次检查机会。
  - 已执行过的并发症检查会记录在面板会话列表中，再次展示时以灰色不可选的普通文本输出，避免重复点击。
