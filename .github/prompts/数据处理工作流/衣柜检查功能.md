...
衣柜检查系统（`check_locker_panel.py`）是erArk游戏中的一个重要功能模块，允许玩家检查和操作各个角色在不同地点（宿舍、更衣室、博士房间）衣柜中的服装物品。该系统涉及服装管理、收藏品获取、以及特殊互动行为等功能。
...
- **核心属性**:
  ...
  - 博士房间按宿舍处理：开启同居服务时助理的 `dormitory` 字段会被改写为博士房间（`assistant_panel.py`），因此她在博士房间用的就是自己的宿舍衣柜，没有独立的博士房间衣柜数据
  - 衣柜的选择统一由 `Script/Design/clothing.py` 的 `get_cloth_locker_type_by_scene()` / `get_cloth_locker_by_type()` 按场景标签路由，面板内不再各自写 `if/elif`
