...
5. 在描写{TargetName}的手部时，将形容词和名词一起使用{hands_s}来替代，可以有多个。如'柔软的手掌'改为'{hands_s}'，不能写为'柔软的{hands_s}'等形式。
## 描述文本的要求
