...
3. 阴茎在前一步的动作里已经夹到{TargetName}的腋下了，不能再描述贴上的动作，只能描述继续摩擦的动作。
4. 在描写{Name}的阴茎或者其他任何对阴茎的指代词的时候，将形容词和名词一起使用{penis_s}来替代，可以有多个。如'粗大的阴茎'改为'{penis_s}'，不能写为'粗大的{penis_s}'等形式。
5. 在描写{TargetName}的腋部时，将形容词和名词一起使用{armpit_s}来替代，可以有多个。如'柔软的腋下'改为'{armpit_s}'，不能写为'柔软的{armpit_s}'等形式。
## 描述文本的要求
