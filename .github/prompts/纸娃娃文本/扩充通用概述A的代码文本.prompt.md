...
2. 每一行的第2列为固定值"action_part_penis_in_body_4_A"。
3. 每一行的第3列是固定值0。
4. 每一行的第4列为前提条件id，用于限定当前指令是在什么情况下发生的。具体的全部限制id以及其情况的详细介绍见说明文件"指令前提分支.md"。
5. 每一行的第5列context为对该指令的描述文本。
# 描述文本
## 描述的基本内容
当前情况为对腋交动作的描述文本，要描述阴茎在腋下进行摩擦腋交的概述。
## 描述文本的具体内容
0. {TargetName}是女性，如果前提条件中没有单独注明，则默认{TargetName}是少女。
1. 这是概述，仅大概描述这个行为，不需要详细描述动作细节或者心理细节。
2. 一定要使用{penis}和{armpit}各一次。
3. 阴茎在前一步的动作里已经贴到{TargetName}的腋下了，不能再描述贴上的动作，只能描述继续摩擦的动作。
4. 在描写{Name}的阴茎或者其他任何对阴茎的指代词的时候，将形容词和名词一起使用{penis}来替代。如'粗大的阴茎'改为'{penis_s}'，不能写为'粗大的{penis_s}'等形式。
5. 在描写{TargetName}的腋部时，将形容词和名词一起使用{armpit}来替代。如'柔软的腋下'改为'{armpit_s}'，不能写为'柔软的{armpit_s}'等形式。
## 描述文本的要求
